...
export function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // This sets the cookie to keep the sidebar state. `document.cookie` is
      // used over `cookieStore`, which Safari does not implement: the call
      // threw there and left setOpen with a rejected promise.
      try {
        document.cookie = `${SIDEBAR_COOKIE_NAME}=${openState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}; samesite=lax`;
      } catch {
        // Cookies can be blocked outright. The sidebar still toggles, it just
        // forgets the state between visits.
      }
    },
    [setOpenProp, open],
  );
